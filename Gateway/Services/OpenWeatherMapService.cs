    ...
    public async Task<GeocodingResponseModels[]> GetCityCoordinatesAsync(string cityName, int responseLimit = 1, string countryCode = "ng")
    {
        ...
        try
        {
            using (HttpClient _client = new HttpClient())
            {
                ...
                var responseReturned = await resp.Content.ReadAsStringAsync();
                var finalOutput = JsonSerializer.Deserialize<GeocodingResponseModels[]>(responseReturned);
                return finalOutput;
            }
        }
        catch (Exception ex)
    ...
    public async Task<CurrentWeatherForecastResponseModel> GetCurrentWeatherForecastAsync(string cityName, string units = "metric")
    {
        try
        {
            using (HttpClient _client = new HttpClient())
            {
                ...
                var requestResponse = await _client.GetAsync($"https://api.openweathermap.org/data/2.5/weather?lat={response[0].Latitude}&lon={response[0].Longitude}&appid={_apiKey}&units={units}");
        ...
        try
        {
            ...
            foreach (string farmLocation in farmLocations)
            {
                using (HttpClient _client = new HttpClient())
                {
                    ...
                    var requestResponse = await _client.GetAsync($"https://pro.openweathermap.org/data/2.5/forecast/climate?lat={response[0].Latitude}&lon={response[0].Longitude}&cnt={daysCount}&appid={_apiKey}&units={units}");
                    ...
                    responseReturned.DailyForecasts.ForEach(df => df.Dates = new Dates
                    {
